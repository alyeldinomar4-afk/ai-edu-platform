...
import { getRelevantChunks } from "./helpers/getRelevantChunks";
...
const ContextualAI = ({
  ...
  console.log("🚀 ~ ContextualAI ~ videoState:", videoState);
  const chunks = videoData?.transcript?.chunks || [];
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      role: "ai",
      text: t("videoPlayer.aiTutor.defaultWelcome"),
      isNew: false,
    },
  ]);
  ...
  useEffect(() => {
    ...
    pauseTimerRef.current = setTimeout(() => {
      ...
      if (
        timeSinceLastPrompt > 10000 &&
        !promptedTimestampsRef.current.has(approximateTime)
      ) {
  ...
  const handleSend = async (e, customText = null) => {
    ...
    try {
      ...
      let action = "chat";
      if (textToSend === t("videoPlayer.aiTutor.explainSectionConfirm"))
        action = "explain-section";
      else if (textToSend === t("videoPlayer.aiTutor.summarizePrompt"))
        action = "summarize";
      else if (textToSend === t("videoPlayer.aiTutor.explainScenePrompt"))
        action = "explain-scene";
      else if (textToSend === t("videoPlayer.aiTutor.createQuizPrompt"))
        action = "create-quiz";
      else if (textToSend === t("videoPlayer.aiTutor.showCodePrompt"))
        action = "show-code";

      const contextChunks = getRelevantChunks(chunks, videoState?.currentTime, {
        minChunks: 12,
        initialContext: 30,
      });
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "ai",
          text: isRTL
            ? "عذراً، حدث خطأ في الاتصال بالمدرب الذكي."
            : "Sorry, I had trouble connecting to the AI Tutor.",
          timestamp: currentTime,
          isNew: true,
        },
      ]);
  ...
  const quickActions = [
    {
      label: t("videoPlayer.aiTutor.summarizePart"),
      text: t("videoPlayer.aiTutor.summarizePrompt"),
      icon: FileText,
      color: "text-blue-400",
    },
    {
      label: t("videoPlayer.aiTutor.explainScene"),
      text: t("videoPlayer.aiTutor.explainScenePrompt"),
      icon: Eye,
      color: "text-emerald-400",
    },
    {
      label: t("videoPlayer.aiTutor.createQuiz"),
      text: t("videoPlayer.aiTutor.createQuizPrompt"),
      icon: Target,
      color: "text-amber-400",
    },
    {
      label: t("videoPlayer.aiTutor.showCode"),
      text: t("videoPlayer.aiTutor.showCodePrompt"),
      icon: Code2,
      color: "text-pink-400",
    },
  ];
            ...
            <AnimatePresence mode="wait">
              {videoState?.isPlaying ? (
                ...
                >
                  ...
                  <span className="text-[10px] font-black text-emerald-500 uppercase tracking-wider whitespace-nowrap">
                    {t("videoPlayer.aiTutor.watchingWithYou", {
                      defaultValue: "LIVE",
                    })}
                  </span>
                </motion.div>
        ...
        <AnimatePresence>
          {messages.map((msg) => (
            ...
            >
              ...
              <div
                className={`group relative p-4 rounded-2xl max-w-[85%] min-w-[30%] text-[14px] leading-relaxed transition-all duration-300 ${
                  msg.role === "ai"
                  ...
                  >
                    <motion.button
                      ...
                      onClick={() =>
                        handleSend(
                          null,
                          t("videoPlayer.aiTutor.explainSectionConfirm"),
                        )
                      }
        ...
        <AnimatePresence>
          {isTyping && (
            ...
            >
              ...
              <div
                className={`bg-[#1E293B] border border-slate-700/50 p-4 rounded-2xl shadow-md ${isRTL ? "rounded-tr-none" : "rounded-tl-none"}`}
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[11px] text-slate-400 font-bold uppercase tracking-widest">
                    {t("videoPlayer.aiTutor.thinking", {
                      defaultValue: "AI is thinking...",
                    })}
                  </span>
                </div>
              </div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-2">
          {quickActions.map((btn, i) => (
            ...
            >
              <btn.icon
                size={14}
                className={`${btn.color} relative z-10 flex-shrink-0 transition-transform group-hover/pill:scale-110`}
              />
