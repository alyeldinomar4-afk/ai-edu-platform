...
const ContextualAI = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 rounded-xl border border-slate-200 overflow-hidden text-slate-900">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages.map((msg) => (
                    <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === 'ai' ? 'bg-primary text-white' : 'bg-slate-200 text-slate-600'}`}>
            ...
            <div className="p-4 bg-white border-t border-slate-200">
                <div className="mb-2 flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                    <button className="text-xs bg-slate-100 hover:bg-slate-200 text-slate-600 px-3 py-1 rounded-full whitespace-nowrap transition-colors">
                ...
                <form onSubmit={handleSend} className="relative">
                    <input
                        ...
                        className="w-full pl-4 pr-12 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all text-sm text-slate-900 placeholder:text-slate-400"
                    />
                    <button
                        type="submit"
