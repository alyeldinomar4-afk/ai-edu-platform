        ...
        {sections.map((section, sIdx) => (
          ...
          >
            ...
            <div className="bg-[#1E293B] px-5 py-3.5 text-xs font-bold text-slate-400 uppercase tracking-widest shadow-inner border-y border-slate-700/50">
              {t("videoPlayer.playlist.section", { number: sIdx + 1 })}:{" "}
              {section.title}
            </div>
              ...
              {section?.lectures?.map((lecture, lIdx) => {
                ...
                const lectureTitle =
                  typeof lecture.title === "string"
                    ? lecture.title
                    : lecture.title?.en || lecture.title?.ar || "Untitled";
                ...
                return (
                  ...
                  >
                    <div className="flex-shrink-0 relative">
                      <div
                        className={`w-[80px] h-[50px] rounded-lg bg-slate-100 dark:bg-slate-800 overflow-hidden shadow-md border transition-all duration-300 ${isActive ? "border-indigo-500/50 shadow-indigo-500/10" : "border-slate-200 dark:border-slate-700 group-hover:border-slate-300 dark:group-hover:border-slate-600"}`}
                      >
                        {lecture.thumbnail ? (
                      ...
                      {/* Status indicator */}
                      <div
                        className={`absolute -bottom-1 z-10 ${isRTL ? "-right-2" : "-left-2"}`}
                      >
                        {isCompleted ? (
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      ...
                      <div className="flex items-center gap-2 mt-1.5 opacity-80">
                        <span
                          className={`text-[11px] font-semibold flex items-center gap-1.5 px-2 py-0.5 rounded-full transition-colors ${isActive ? "bg-indigo-500/20 text-indigo-300" : "bg-slate-800 text-slate-400 group-hover:bg-slate-700"}`}
                        >
