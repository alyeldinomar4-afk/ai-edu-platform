...
const CourseDetailsPage = () => {
  ...
  const handleSubmitReview = () => {
    ...
    if (!reviewText.trim()) {
      setReviewError("Please write your feedback before submitting.");
      return;
    }
    setReviewError("");
    setReviews((prev) => [
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <div className="flex-1 min-w-0">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 leading-tight wrap-break-word">
                {course.title}
              </h1>
              ...
              <div className="flex flex-wrap items-center gap-4 sm:gap-6 text-xs sm:text-sm text-slate-300 dark:text-slate-400">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    {t("courseDetails.numReviews", { count: course.reviews })}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                      ...
                      <div className="space-y-4">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-1 mb-4">
                          ...
                          <span className="text-sm text-slate-500 dark:text-slate-400">
                            {lectures.length} {t("courseDetails.lectures")} •{" "}
                        ...
                        {lectures.length > 0 ? (
                          <div className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                            ...
                            <div className="divide-y divide-slate-100 dark:divide-slate-800">
                              {lectures.map((lecture, idx) => (
                                <div
                                  key={lecture.id}
                                  className="px-4 py-3 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group"
                                >
                          ...
                          <div className="text-center py-12 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                            ...
                            <p className="text-slate-500 dark:text-slate-400">
                              {t(
                                "courseDetails.noLectures",
                                "No lectures available yet for this course.",
                              )}
                            </p>
                          </div>
                        )}
                          ...
                          <div className="absolute inset-0 z-10 bg-white/80 dark:bg-slate-900/90 backdrop-blur-sm flex flex-col items-center justify-center rounded-2xl">
                            ...
                            >
                              ...
                              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">
                                {t("courseDetails.locked.title")}
                              </h3>
                              <p className="text-sm text-slate-500 dark:text-slate-400 mb-4 max-w-[250px] mx-auto">
                                {t(
                                  "courseDetails.locked.reviews_subtitle",
                                  "Purchase this course to see what other students are saying and leave your own feedback.",
                                )}
                              </p>
                              <Button
                                onClick={() => navigate(`/checkout/${id}`)}
                                size="sm"
                                className="shadow-[0_4px_14px_0_rgb(79,70,229,0.39)] bg-primary hover:bg-primary-dark"
                              >
                        ...
                        >
                          ...
                          <div className="flex flex-col md:flex-row items-center gap-8 bg-slate-50 dark:bg-slate-800/50 p-8 rounded-2xl border border-slate-100 dark:border-slate-800">
                            <div className="text-center md:border-r md:border-slate-200 dark:md:border-slate-700 md:pr-12">
                              ...
                              <div className="flex justify-center gap-1 mb-2">
                                {[1, 2, 3, 4, 5].map((i) => (
                                  <Star
                                    key={i}
                                    className={`w-5 h-5 ${i <= Math.round(course.rating) ? "fill-yellow-400 text-yellow-400" : "text-slate-300 dark:text-slate-600"}`}
                                  />
                                ))}
                              </div>
                            ...
                            <div className="flex-1 w-full space-y-3">
                              {[5, 4, 3, 2, 1].map((rating) => (
                                ...
                                >
                                  <span className="w-12 font-medium text-slate-600 dark:text-slate-400">
                                    {rating} Stars
                                  </span>
                          ...
                          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border-2 border-primary/10 shadow-sm">
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">
                              {t("courseDetails.leaveReview")}
                            </h3>
                            <div className="space-y-4">
                              <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                  {t("courseDetails.ratePrompt")}
                                </label>
                                <div className="flex gap-1">
                                  ...
                                  {reviewRating > 0 && (
                                    <span className="ml-2 self-center text-sm font-semibold text-yellow-500">
                                      {reviewRating === 1
                                        ? t("courseDetails.ratings.poor")
                                        : reviewRating === 2
                                          ? t("courseDetails.ratings.fair")
                                          : reviewRating === 3
                                            ? t("courseDetails.ratings.good")
                                            : reviewRating === 4
                                              ? t(
                                                  "courseDetails.ratings.veryGood",
                                                )
                                              : t(
                                                  "courseDetails.ratings.excellent",
                                                )}
                                    </span>
                                  )}
                                </div>
                              ...
                              <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                  {t("courseDetails.feedbackPrompt")}
                                </label>
                                <textarea
                                  value={reviewText}
                                  onChange={(e) => {
                                  ...
                                  placeholder={t(
                                    "courseDetails.feedbackPlaceholder",
                                  )}
                                  rows="4"
                              ...
                              {reviews.map((r, i) => (
                                ...
                                >
                                  <div className="w-12 h-12 rounded-full bg-slate-200 dark:bg-slate-800 shrink-0 overflow-hidden">
                                    <img
                                      src={`https://ui-avatars.com/api/?name=${r.name}&background=random`}
                                      alt={r.name}
                                    />
                                  </div>
                                      ...
                                      <div>
                                        ...
                                        <div className="flex gap-0.5 mt-1">
                                          {[1, 2, 3, 4, 5].map((star) => (
                                            <Star
                                              key={star}
                                              className={`w-3 h-3 ${star <= r.rating ? "fill-yellow-400 text-yellow-400" : "text-slate-200 dark:text-slate-700"}`}
                                            />
                                          ))}
                                        </div>
                                      </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-lg border border-slate-100 dark:border-slate-800 space-y-6 sticky top-24 transition-colors">
              ...
              <div>
                <div className="flex items-end gap-2 mb-4">
                  ...
                  {course.price > 0 && course.discount > 0 && (
                    <>
                      ...
                      <span className="text-sm font-semibold text-green-600 dark:text-green-400 mb-1 ml-auto">
                        {course.discount}% OFF
                      </span>
                    </>
                  )}
                ...
                user?.name === course.instructor.fullName ? (
                  <Link to={`/instructor/dashboard`}>
                    <Button
                      className="w-full mb-3 shadow-[0_4px_14px_0_rgb(59,130,246,0.39)] hover:shadow-[0_6px_20px_rgba(59,130,246,0.23)] hover:-translate-y-0.5 transition-all"
                      size="lg"
                    >
                  ...
                  <Link to={`/checkout/${id}`}>
                    <Button
                      className="w-full mb-3 shadow-[0_4px_14px_0_rgb(79,70,229,0.39)] hover:shadow-[0_6px_20px_rgba(79,70,229,0.23)] hover:-translate-y-0.5 transition-all"
                      size="lg"
                    >
              ...
              <div className="space-y-4 pt-6 border-t border-slate-100 dark:border-slate-800">
                ...
                <ul className="space-y-3 text-sm text-slate-600 dark:text-slate-300">
                  ...
                  <li className="flex items-center gap-3">
                    ...
                    {course.lessons} {t("courseDetails.articlesResources")}
                  </li>
                  <li className="flex items-center gap-3">
